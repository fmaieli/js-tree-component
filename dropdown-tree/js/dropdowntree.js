    ...
    $.fn.DropDownTree = function (options) {
        ...
        function RenderData(data, element) {
            for (var i = 0; i < data.length; i++) {
                ...
                if (!element.is("li")) {
                    element.append('<li id="' + data[i].Id + '"' + dataAttrs + '>'
                        + (options.multiSelect ? '<label class="ui-corner-all"><input type="checkbox" value="' + data[i].Id + '"><span>' + data[i].title + '</span><a href="' + ((typeof data[i].href != "undefined" && data[i].href != null) ? data[i].href : '#') + '"></a></label>' : '')
                        + '</li>');
                    if (data[i].data != null && typeof data[i].data != "undefined") {
                        $("#" + data[i].Id).append("<ul style='display:none'></ul>");
                        $("#" + data[i].Id).find("a").first().prepend('<span class="arrow"></span>');
                        RenderData(data[i].data, $("#" + data[i].Id).find("ul").first());
                    } else if (options.addChildren) {
                        $("#" + data[i].Id).find("a").first().prepend('<span class="arrow"></span>');
                    }
                }
                else {
                    element.find("ul").append('<li id="' + data[i].Id + '"' + dataAttrs + '>'
                        + (options.multiSelect ? '<label class="ui-corner-all"><input type="checkbox" value="' + data[i].Id + '"><span>' + data[i].title + '</span><a href="' + ((typeof data[i].href != "undefined" && data[i].href != null) ? data[i].href : '#') + '"></a></label>' : '')
                        + '</li>');
                    if (data[i].data != null && typeof data[i].data != "undefined") {
                        $("#" + data[i].Id).append("<ul style='display:none'></ul>");
        ...
        $(options.element).on("click", "input[type='checkbox']", function (e) {
            ...
            var childrens = e.currentTarget.parentElement.parentElement.getElementsByTagName("ul");
        ...
        $(options.element).append('<button class="ui-multiselect ui-widget ui-state-default ui-corner-all float-left" data-toggle="dropdown" style="width: 146px; height: 28px; padding-top: 3px; padding-bottom: 3px; font-size: 14px; margin-top: 0px;"><span class="dropdowntree-name">' + options.title + '</span><span class="caret"></span></button>');
        $(options.element).append('<ul style="max-height: ' + options.maxHeight + 'px; border: 1px solid #000000;" class="dropdown-menu ui-multiselect-checkboxes ui-helper-reset" aria-labelledby="dropdownMenu1"></ul>');
...
function GetTreeElementsChecked(element) {
    ...
    inputValuesConcatenated = inputValuesConcatenated.substring(0, inputValuesConcatenated.length - 1);
